	...
	//TODO:bug 不能返回多个父级
	    ...
	    //, checkBool=true;
            for (var i = 0; i < this.length; i++) {
                valueA = F.trim(_preFormCallBack(value, this[i], i)).split(/\s+/)
                ...
                while(j--){
                    //不使用正则，慢了点
                    if(classStr.indexOf(" " + valueA[j] + " ") === -1){
                        //checkBool = false;
                        return false;
                        //break;
                    }
        ...
        addClass: function (value) {
            var valueA, classStr,classNames,setClass;
            for (var i = 0; i < this.length; i++) {
                valueA = F.trim(_preFormCallBack(value, this[i], i)).split(/\s+/);
                classStr = F.trim(this[i].className);
                for (var j = 0; j < valueA.length; j++) {
                    //if (!F.all(this[i]).hasClass(valueA[j])) {
                    //处理value值多空格情况
                    ...
                    //if ( this[i].nodeType === 1 ) {
                        if ( !this[i].className && classNames.length === 1 ) {
                            ...
                            classStr = F.trim( setClass );
                            this[i].className =classStr;
                        }
		...
		removeClass: function (value) {
			...
			for (var i = 0; i < this.length; i++) {
				...
				for (var j = 0; j < valueA.length; j++) {
                    //这样快点
                    classStr = classStr.replace(" " + valueA[j] + " ", " ");
				}
				this[i].className = F.trim(classStr);
			}
			return this;
		},
        ...
        */
        ajax : function (param) {
            ...
            if(param.type === "jsonp"){
                var callback = param.jsonp ? param.jsonp : "callback";
                var RandomFn = "tc" + parseInt(100000000000*Math.random());
