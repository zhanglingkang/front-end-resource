	...
	function initShowDetail () {
		...
		jDetailBtn.click(function () {
			var prodId = $(this).attr("data-prodId"),
				...
				contractInfo = "";
			if(prodData.imgSrc){
				if(prodData.imgSrc.indexOf("/uploadImg")>-1){//服务器上的图片，路径有些问题
					$("img",jProdDetail).attr("src","/buyAndSellMarket"+prodData.imgSrc);
				}else{
			...
			if(prodData.mobile && prodData.mobile != "null"){
				contractInfo+=(prodData.mobile + " ");
			}
			if(prodData.qq && prodData.qq != "null"){
